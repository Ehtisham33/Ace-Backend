...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
            ...
            if typing and not message_text and not file_url:
                await self.channel_layer.group_send(
                    self.room_group_name,
                    {
                        ...
                        'room_id': sender_id  # ✅ Always sender_id
                    }
                )
                return

            # Detect file type
            file_type = self.detect_file_type(file_url) if file_url else None
            ...
            message_event = {
                ...
                'typing': False,
                'room_type': self.room_type,
                'room_id': sender_id  # ✅ always sender ID
            }

            # Send to sender group
            await self.channel_layer.group_send(
                f"{self.room_type}_chat_{sender_id}",
                message_event
            )

            # Send to receiver group
            await self.channel_layer.group_send(
                f"{self.room_type}_chat_{receiver_id}",
                message_event
            )
    ...
    def create_notification(self, sender, receiver, message_text):
        if sender != receiver:
            Notification.objects.create(
                ...
                message=Message.objects.filter(sender=sender, receiver=receiver).last()
            )
